...
.PHONY: analytics-pipeline-devstack-test build-courses clean-marketing-sync \
        ...
        dev.restart-container dev.restart-devserver \
...
dev.restart-container.%: ## Restart specific services' containers (+-separated).
	docker-compose restart $$(echo $* | tr + " ")
...
$(addsuffix -dbshell, $(EDX_SERVICES_LIST)): %-dbshell: dev.dbshell.%
